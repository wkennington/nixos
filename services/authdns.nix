...
in
{
  ...
  networking.firewall.extraCommands = ''
    ...
    iptables -t mangle -X nixos-knot-output || true
    iptables -t mangle -N nixos-knot-output
    iptables -t mangle -A nixos-knot-output -o lo -j RETURN
    iptables -t mangle -A nixos-knot-output -j MARK --set-mark 0x20
    iptables -t mangle -A OUTPUT -m owner -p tcp --dport 53 --uid-owner knot -j nixos-knot-output
    iptables -t mangle -A OUTPUT -m owner -p udp --dport 53 --uid-owner knot -j nixos-knot-output
